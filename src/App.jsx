...
export default function LanguageTutor() {
  ...
  // 대화 목록 — localStorage에서 복원
  const [convList, setConvList] = useState(() => {
    try { return JSON.parse(localStorage.getItem("lingua_conv_list") || "[]"); } catch { return []; }
  });
  const [showConvDelete, setShowConvDelete] = useState(null); // 삭제 확인 중인 id
...
4. Keep responses natural and encouraging.`;
  ...
  function saveAsText() {
    ...
    setConvList(prev => {
      const filtered = prev.filter(e => e.id !== ts);
      return [entry, ...filtered].slice(0, 100); // 최대 100개
    });
  }
  ...
  // ---------- 목록에서 대화 다시 보기 (텍스트 뷰어) ----------
  function viewConv(entry) {
    const newWin = window.open("", "_blank");
    if (newWin) {
      newWin.document.write(`<pre style="font-family:sans-serif;padding:20px;white-space:pre-wrap;">${entry.content}</pre>`);
      newWin.document.title = entry.title;
    } else {
      // 팝업 차단 시 다운로드
      const a = document.createElement("a");
            ...
            {activeTab === "history" && (
              <>
                ...
                <div style={{ display: "flex", gap: "8px", marginBottom: "4px" }}>
                  ...
                  {convList.length > 0 && (
                    <button
                      onClick={() => { if (window.confirm("목록을 전체 삭제할까요?")) setConvList([]); }}
                      style={{
                        background: "rgba(248,113,113,0.1)", border: "1px solid rgba(248,113,113,0.3)",
                ...
                {convList.length === 0 ? (
                  <div style={{ textAlign: "center", color: "#555", marginTop: "40px" }}>
                    ...
                    <p style={{ fontSize: "11px", color: "#444", marginTop: "4px" }}>저장 버튼을 누르거나<br/>파일을 불러오세요</p>
                  </div>
                ) : (
                  convList.map(entry => (
                    <div key={entry.id} style={{
                      ...
                      <div style={{ display: "flex", alignItems: "center", gap: "6px", marginBottom: "6px" }}>
                        ...
                        {/* 레벨 뱃지 */}
                        <span style={{
                          background: LEVEL_COLORS[entry.level] + "33",
                          border: `1px solid ${LEVEL_COLORS[entry.level]}66`,
                      ...
                      {showConvDelete === entry.id ? (
                        <div style={{ display: "flex", gap: "6px" }}>
                          ...
                          <button onClick={() => { setConvList(p => p.filter(e => e.id !== entry.id)); setShowConvDelete(null); }}
                            style={{ background: "rgba(248,113,113,0.2)", border: "1px solid rgba(248,113,113,0.5)", borderRadius: "8px", padding: "4px 10px", color: "#f87171", cursor: "pointer", fontSize: "11px" }}>삭제</button>
                          <button onClick={() => setShowConvDelete(null)}
                            style={{ background: "rgba(255,255,255,0.06)", border: "1px solid rgba(255,255,255,0.1)", borderRadius: "8px", padding: "4px 10px", color: "#aaa", cursor: "pointer", fontSize: "11px" }}>취소</button>
                        </div>
                      ) : (
                        <div style={{ display: "flex", gap: "6px" }}>
                          <button onClick={() => viewConv(entry)}
                          ...
                          <button onClick={() => setShowConvDelete(entry.id)}
